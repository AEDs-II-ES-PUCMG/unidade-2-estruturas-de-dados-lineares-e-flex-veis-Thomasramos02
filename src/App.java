...
public class App {
    ...
    /** Lista de pedidos aguardando processamento */
    ...
    static int menu() {
        ...
        System.out.println("7 - Exibir os pedidos que apresentam um determinado produto");
    ...
    public static void finalizarPedido(Pedido pedido) {
        if (pedido != null && !pedido.getItensDoPedido().vazia()) {
            listaPedidos.inserirFim(pedido);
            cabecalho();
    ...
    public static void listarProdutosPedidosRecentes() {
        ...
        if (listaPedidos.vazia()) {
            ...
            int inicio = Math.max(0, listaPedidos.tamanho() - 5);

            for (int i = inicio; i < listaPedidos.tamanho(); i++) {
                Pedido pedido = listaPedidos.obter(i);
    ...
    /**
     * Exibe os pedidos que possuem um produto com a descrição informada pelo usuário.
     */
    public static void filtrarPorProduto() {
        cabecalho();
        System.out.println("FILTRAR PEDIDOS POR PRODUTO");
        System.out.println("===========================\n");
        ...
        Produto produtoBusca = new ProdutoNaoPerecivel(descricao, 1.0, 0.1);
        ItemDePedido itemBusca = new ItemDePedido(produtoBusca, 0, 0.0);
        Comparator<ItemDePedido> comparadorDescricao = (item1, item2) -> item1.getProduto().descricao.compareToIgnoreCase(item2.getProduto().descricao);

        Predicate<Pedido> pedidoTemProduto = pedido -> pedido.getItensDoPedido().buscarPor(comparadorDescricao, itemBusca) != null;

        Lista<Pedido> pedidosFiltrados = listaPedidos.filtrar(pedidoTemProduto);
        ...
        if (pedidosFiltrados.vazia()) {
            ...
            for (int i = 0; i < pedidosFiltrados.tamanho(); i++) {
                System.out.println(pedidosFiltrados.obter(i));
                System.out.println();
            }
        }
    }
